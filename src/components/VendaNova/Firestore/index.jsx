...
export default function Firestore(){
    const [novaNota, setNovaNota] = useState({
        ...
        pagoPrefeitura: false, pagoFornecedor: false, arquivo: '' ,timeStamp: null
    })
    const [updatedNota, setUpdatedNota] = useState({
        campanha: '', tipo: '', pppi: '', fornecedor: '',
        descricao: '', valor: '', notafiscal: '', processo: '',
        statusNota: '', pagoPrefeitura: false, pagoFornecedor: false, arquivo: ''
    })
    ...
    const AddNota = async (e) => {
        ...
        if (editarNota) {
          const notaRef = doc(db, 'notas-venda-nova', editarNota.id)
          ...
          closeModal();
            ...
            const storageRef = ref(storage, novaNota.arquivo)

            uploadBytes(storageRef, file).then((snapshot) => {
                console.log('upload realizado', snapshot.state)
            }) */
            await addDoc(collection(db, 'notas-venda-nova'), {
          ...
          setNovaNota({
            ...
            fornecedor: '', descricao: '', valor: '',
            notafiscal: '', processo: '', statusNota: 'naolancada',
            pagoPrefeitura: false, pagoFornecedor: false, arquivo: ''
          });
        } else {
      ...
      const MostrarNotas = async () => {
        const colecao = collection(db, 'notas-venda-nova');
        const q = query(
          colecao,
      ...
      const deletarNota = async (id) => {
        await deleteDoc(doc(db, 'notas-venda-nova', id));
        MostrarNotas();
      };
      ...
      const updateNotaInFirestore = async (notaId) => {
        const notaRef = doc(db, 'notas-venda-nova', notaId);
                        ...
                        <div className={`${styles.formControl}`}>
                            ...
                            <div className={`flex gap-1 mb-5`}>
                                <input 
                                    type="file" 
                                    name="arquivo" 
                                    value={novaNota.arquivo}
                                    id="arquivo"
                                    onChange={(e) => setNovaNota({...novaNota, arquivo: e.target.value})}
                                    placeholder={`Anexo`}
                    ...
                    <caption className={`caption-top mt-2`}>
                        ...
                        <thead className={`bg-gray-50 border-b-2`}>
                            <tr>
                                ...
                                <th className={`p-3 text-sm font-semibold tracking-wide text-center`}>Anexo</th>
                        ...
                        <tbody className={`divide-y divide-gray-100`}>
                            {notas.map((nota, id) => (
                                ...
                                <tr key={id} className={`${id % 2 == 0 ? 'bg-white text-gray-700' : 'bg-gray-400 text-white'}`}>
                                    ...
                                    <td className={`p-3 text-sm whitespace-wrap text-center`}>{nota.arquivo}</td>
                                    <td>
                                        <button 
                                            type="button" 
